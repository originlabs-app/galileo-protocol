...
Two passport programmes, two Gommino sub-ranges, two carriers, two registries — documented. Whether a single pair will ever carry both carriers is not. If, or when, it does, which attestation becomes the source of truth? That question is no longer theoretical. It is the exact interoperability problem the EU's Digital Product Passport framework will put in front of every maison — and Tod's just became its most concrete public case.
