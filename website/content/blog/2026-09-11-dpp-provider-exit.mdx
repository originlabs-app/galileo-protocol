...
The [Ecodesign for Sustainable Products Regulation, Articles 9 and 10](https://eur-lex.europa.eu/eli/reg/2024/1781/2024-06-28/eng), links product-specific DPP requirements to delegated acts. It provides for open standards, data exchange between systems without supplier lock-in, and a backup copy made available by the economic operator through a DPP service provider. The availability period is set in each delegated act and must be at least the expected lifetime of the product, under Article 9(2), point (i). Articles 27(1)(c) and 29 make the manufacturer and the importer responsible for ensuring that a backup copy of the most up-to-date version of the passport is stored by a DPP service provider, and Annex III requires the passport itself to carry the reference of the provider hosting that copy. Changing provider therefore means updating that reference, not only moving the data.

[Article 11, first paragraph, point (e)](https://eur-lex.europa.eu/eli/reg/2024/1781/2024-06-28/eng) requires availability for the specified period, including after insolvency, liquidation or cessation of activity in the Union of the economic operator responsible for creating the passport. That subject matters: the provision does not itself describe an automatic rescue of a failed software supplier. The second paragraph of the same article also bars a DPP service provider from selling, reusing or processing passport data beyond what the storage or processing service requires, unless specifically agreed with the economic operator. That protection is already in the regulation; the contract should reference it rather than reinvent it.
...
Agree how much recent work the brand could tolerate losing, and how quickly it needs access restored. Those are service requirements to negotiate for the actual operation. They should not be invented from a generic industry benchmark. The regulation already sets a floor: Articles 27(1)(c) and 29 require a backup copy of the most up-to-date version of the passport, stored by a DPP service provider under Article 10(4). Negotiate the operational detail above that floor, not below it.
...
- **Recovery access:** where the copy is kept, who can retrieve it and how access works if the ordinary account is unavailable, plus the party named in the passport as hosting the backup copy and who updates that reference at handover.
- **Service continuity:** the agreed update frequency, recoverable period, restoration target and responsibilities during a transition.
